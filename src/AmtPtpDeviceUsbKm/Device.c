...
AmtPtpDeviceUsbKmCreateDevice(_Inout_ PWDFDEVICE_INIT DeviceInit)
{
    ...
    // WdfDeviceInitAssignSDDLString requires a *named* device object.
    // WdfDeviceInitAssignName(DeviceInit, NULL) does NOT satisfy this for
    // an FDO/filter (NULL only triggers auto-naming for PDOs/control
    // objects) - calling it here would silently leave the device unnamed
    // and WdfDeviceCreate below would fail with STATUS_INVALID_SECURITY_DESCR,
    // which is exactly the device-start failure seen in Device Manager
    // after the first version of this fix. The correct way to get an
    // auto-generated name for an FDO is FILE_AUTOGENERATED_DEVICE_NAME.
    // We don't need a specific symbolic device name of our own - only the
    // device *interface* path is used by the GUI, via SetupDi.
